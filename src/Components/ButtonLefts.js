...
const initializeChart = (data) => {
  ...
  filteredData.forEach((item) => {
    amountSum += parseInt(item.value) - parseInt(item.out_value) + parseInt(item.in_value)
  });

  let categoryValue6 = amountSum;

  finalData.push({ category: categoryValue6, value: 2.6 });

  let root = am5.Root.new("ch-lefts");
...
const ButtonLefts = ({ data }) => {
  ...
  return (
    <div class="chblock-lefts">
    <div class="chtl-lefts">Остаток</div>
    <div id="ch-lefts"></div>
    </div>
    );
};

ButtonLefts.propTypes = {
  data: PropTypes.array.isRequired,
};

export default ButtonLefts;
